...
import { getRecipeByIdApi, updateRecipeApi, uploadImageApi } from '../../api/recipe.api';
...
const EditRecipePage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await updateRecipeApi(id, { ...data, ingredients: vi, instructions: vn, toolsUsed, image: imageUrl, prepTime: Number(data.prepTime), cookTime: Number(data.cookTime), servings: Number(data.servings) });
      toast.success('Recipe updated!'); navigate(`/recipes/${id}`);
    } catch (err) { setUploading(false); toast.error(err.response?.data?.message || 'Failed to update recipe'); }
  };
      ...
      <div className="page-enter">
        <div className="bg-parchment border-b border-linen py-8">
          <div className="max-w-2xl mx-auto px-6">
            ...
            <h1 className="font-heading text-4xl">Edit recipe</h1>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="mb-10">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea {...register('description', { required: 'Description is required' })} rows={3} className={`${inputClass} resize-none`} />
                  {errors.description && <span className="text-xs text-red-500 mt-1 block">{errors.description.message}</span>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="editorial-label block mb-1.5">Category</label><select {...register('category', { required: 'Required' })} className={inputClass}><option value="">Select...</option>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            ...
            <div className="mb-10">
              ...
              {ingredients.map((ing, i) => (
                <div key={i} className="grid grid-cols-[120px_1fr_auto] gap-2 items-center mb-2">
                  <input type="text" placeholder="e.g. 2 cups" value={ing.amount} onChange={e => updateIngredient(i,'amount',e.target.value)} className={`${inputClass} w-full min-w-0`} />
                  <input type="text" placeholder="e.g. Rice" value={ing.name} onChange={e => updateIngredient(i,'name',e.target.value)} className={`${inputClass} w-full min-w-0`} />
                  {ingredients.length > 1 && <button type="button" onClick={() => removeIngredient(i)} className="border border-linen text-gray-400 rounded-lg px-3 py-3 text-xs hover:border-sand hover:text-gray-600 transition-colors self-stretch">×</button>}
                </div>
              ))}
